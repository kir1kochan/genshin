...
Hud::Hud(Player* player)
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // 初始化技能冷却进度条（蒙版）
        cocos2d::Sprite* cooldownSprite = cocos2d::Sprite::create("imageSkill/Covery.pgn");
...
void Hud::equipSkill(int skillSlot, std::string skillName)
{
    ...
    if (skillIcon) {
        ...
        // 确保技能图标在进度条上方
        skillIcon->setLocalZOrder(1);  // 将技能图标置于上方
    }
    ...
    skillCooldownBars[skillSlot]->setContentSize(cocos2d::Size(50, 50));
...
void Hud::useSkill(int skillSlot, float cdTime) {
    ...
    // 动态更新冷却进度条
    cocos2d::Action* cooldownAction = cocos2d::ProgressTo::create(cdTime, 0);
